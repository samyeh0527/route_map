    ...
    def _on_track_click(self, event):
        ...
        try:
            ...
            if nearest_idx is None:
                return
            print(f'check full_data type {type(self.full_data)}')
            if type(self.full_data) == list:
                # 如果 full_data 是 list，則轉 DataFrame
                if len(self.full_data) > 1 :
                    for i in range(len(self.full_data)):
                        self.full_data_df_list.append(pd.DataFrame(self.full_data[i]))
                    #如果兩個以上就用迴圈
            else:
                self.full_data= pd.DataFrame(self.full_data[0])
                print(f'conver full_data type {type(self.full_data)} \n {len(self.full_data)}')

            try:
                print(f'[DEBUG] \n{self.full_data} , \n[DEBUG full_data type]{type(self.full_data)}')
                print(f'[DEBUG nearest_idx] \n{nearest_idx} \n {type(nearest_idx)}')
            except Exception as e:
                print(f"獲取點擊位置經緯度時出錯: {str(e)}")
                return
